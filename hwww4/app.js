class MNISTApp {
    ...
    async onTrainClassifier() {
        ...
        try {
            ...
            const visCbCls = tfvis.show.fitCallbacks(
            ...
            const clsCb = {
                ...
                onEpochEnd: async (epoch, logs) => {
                    this.log('Classifier epoch ' + (epoch+1) + ' — loss: ' + logs.loss.toFixed(4) + '  val_loss: ' + logs.val_loss.toFixed(4) + '  val_acc: ' + logs.val_acc.toFixed(4));
                    if (visCbCls.onEpochEnd) await visCbCls.onEpochEnd(epoch, logs);
                }
            };
    ...
    async onTrainAutoencoder() {
        ...
        try {
            ...
            const makeCb = (name) => {
                const visCb = tfvis.show.fitCallbacks(
                ...
                return {
                    ...visCb,
                    onEpochEnd: async (epoch, logs) => {
                        this.log(name + ' epoch ' + (epoch+1) + ' — loss: ' + logs.loss.toFixed(6) + '  val_loss: ' + logs.val_loss.toFixed(6));
                        if (visCb.onEpochEnd) await visCb.onEpochEnd(epoch, logs);
                    }
                };
            };
