 ...
 *   Decoder: UpSampling2D → Conv2D(32) → Conv2D(1, sigmoid)
...
class DenoiserApp {
        ...
        try {
            ...
            for (let epoch = 0; epoch < epochs; epoch++) {
                ...
                for (let b = 0; b < nBatch; b++) {
                    ...
                    const rawLoss = await model.trainOnBatch(noisyBatch, cleanBatch);
                    // trainOnBatch returns a number (MSE scalar)
                    epochLoss += Array.isArray(rawLoss) ? rawLoss[0] : rawLoss;
    ...
    createModel(poolType = 'max') {
        ...
        // ── Decoder ──────────────────────────────────────────
        // UpSampling2D restores spatial dims: [14,14] → [28,28]
        // We use plain Conv2D after upsampling (avoids checkerboard artifacts
        // and the "black square" bug that Conv2DTranspose can produce here).
        ...
        // Final conv: 1 channel, sigmoid → pixel values in [0, 1]
        model.add(tf.layers.conv2d({
    ...
    // ─────────────────────────────────────────────────────────
    //  Render denoising results — layout matches the homework slide:
    //
    //    Row 1 (noisy):     [img1] [img2] [img3] [img4] [img5]   ← +Noise input
    //    Row 2 (max pool):  [img1] [img2] [img3] [img4] [img5]   ← Max denoised
    //    Row 3 (avg pool):  [img1] [img2] [img3] [img4] [img5]   ← Avg denoised
    //
    //  PSNR is shown under each image; winner per column gets a green border.
    // ─────────────────────────────────────────────────────────
    renderPreview(cleanArr, noisyArr, maxArr, avgArr) {
        ...
        // Helper: create a canvas cell with an optional PSNR badge below
        const makeCell = (imageData, psnrText, badgeClass, isWinner) => {
            const cell = document.createElement('div');
            cell.className = 'img-cell';

            const canvas = document.createElement('canvas');
            const tensor = tf.tensor(imageData);
            this.dataLoader.draw28x28ToCanvas(tensor, canvas, 3);  // 3× → 84px
            tensor.dispose();
            cell.appendChild(canvas);

            if (psnrText) {
                const badge = document.createElement('span');
                badge.className = `psnr-badge ${badgeClass}${isWinner ? ' psnr-winner' : ''}`;
                badge.textContent = psnrText;
                cell.appendChild(badge);
            }
            return cell;
        };

        // Helper: build one labelled row of 5 images
        const makeRow = (label, color, cellsArray) => {
            const wrapper = document.createElement('div');
            wrapper.className = 'preview-row-group';
            ...
            row.className = 'img-strip';
            cellsArray.forEach(c => row.appendChild(c));
            wrapper.appendChild(row);
            return wrapper;
        };

        // Compute PSNR per image first (needed for winner highlight)
        const psnrNoisyArr = [], psnrMaxArr = [], psnrAvgArr = [];
        for (let i = 0; i < 5; i++) {
            psnrNoisyArr.push(this.computePSNR(cleanArr[i], noisyArr[i]));
            psnrMaxArr.push(maxArr ? this.computePSNR(cleanArr[i], maxArr[i]) : null);
            psnrAvgArr.push(avgArr ? this.computePSNR(cleanArr[i], avgArr[i]) : null);
            totalPsnrNoisy += psnrNoisyArr[i];
            if (psnrMaxArr[i] !== null) totalPsnrMax += psnrMaxArr[i];
            if (psnrAvgArr[i] !== null) totalPsnrAvg += psnrAvgArr[i];
        }

        // ── Row 1: Noisy inputs ──────────────────────────────
        const noisyCells = cleanArr.map((_, i) =>
            makeCell(noisyArr[i], `${psnrNoisyArr[i].toFixed(1)} dB`, 'psnr-noise', false)
        );
        container.appendChild(makeRow('+ Noise (input)', '#6a1b9a', noisyCells));

        // ── Row 2: Max Pooling denoised ──────────────────────
        if (maxArr) {
            const maxCells = cleanArr.map((_, i) => {
                const winner = psnrAvgArr[i] === null || psnrMaxArr[i] >= psnrAvgArr[i];
                return makeCell(maxArr[i], `${psnrMaxArr[i].toFixed(1)} dB`, 'psnr-max', winner);
            });
            container.appendChild(makeRow('Max Pooling denoised', '#1565c0', maxCells));
        }

        // ── Row 3: Avg Pooling denoised ──────────────────────
        if (avgArr) {
            const avgCells = cleanArr.map((_, i) => {
                const winner = psnrMaxArr[i] === null || psnrAvgArr[i] > psnrMaxArr[i];
                return makeCell(avgArr[i], `${psnrAvgArr[i].toFixed(1)} dB`, 'psnr-avg', winner);
            });
            container.appendChild(makeRow('Avg Pooling denoised', '#c62828', avgCells));
        }
