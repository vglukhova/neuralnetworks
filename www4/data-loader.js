...
class MNISTDataLoader {
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    // NOTE: do NOT use tf.tidy() — xs and ys must survive after this call
                    const raw = tf.tensor2d(pixels);
                    const xs  = raw.div(255).reshape([labels.length, 28, 28, 1]);
                    raw.dispose();

                    // One-hot encode labels to depth 10
    ...
    splitTrainVal(xs, ys, valRatio = 0.1) {
        // NOTE: do NOT use tf.tidy() here — returned tensors must survive after the call
    ...
    getRandomTestBatch(xs, ys, k = 5) {
        // NOTE: do NOT use tf.tidy() — returned tensors must survive after the call
