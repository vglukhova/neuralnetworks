class MNISTDataLoader {
    ...
    addNoise(images, noiseFactor = 0.4) {
        return tf.tidy(() => {
    ...
    drawComparison(original, noisy, denoised, canvas, scale = 2) {
        return tf.tidy(() => {
            const ctx = canvas.getContext('2d');
            canvas.width = 28 * 3 * scale;
            canvas.height = 28 * scale;
            
            // Create temp canvases for each image
            const tempCanvas1 = document.createElement('canvas');
            const tempCanvas2 = document.createElement('canvas');
            const tempCanvas3 = document.createElement('canvas');
            
            this.draw28x28ToCanvas(original, tempCanvas1, 1);
            this.draw28x28ToCanvas(noisy, tempCanvas2, 1);
            this.draw28x28ToCanvas(denoised, tempCanvas3, 1);
            
            ctx.imageSmoothingEnabled = false;
            ctx.drawImage(tempCanvas1, 0, 0, 28 * scale, 28 * scale);
            ctx.drawImage(tempCanvas2, 28 * scale, 0, 28 * scale, 28 * scale);
            ctx.drawImage(tempCanvas3, 56 * scale, 0, 28 * scale, 28 * scale);
            ...
            ctx.strokeText('Noisy', 28 * scale + 5, 20);
            ctx.fillText('Noisy', 28 * scale + 5, 20);
            ctx.strokeText('Denoised', 56 * scale + 5, 20);
            ctx.fillText('Denoised', 56 * scale + 5, 20);
        });
    }
